...
export const PurchasesHistoryTable = ({
    ...
    columnHelper.accessor((row) => row.purchase.status, {
      ...
      cell: (info) => {
        ...
        if (status === "partial_return") {
          return (
            <span className="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-800 dark:border dark:border-amber-800/60 dark:bg-amber-950/60 dark:text-amber-300">
              Devolución parcial
            </span>
          );
        }
        ...
        if (status === "returned") {
          return (
            <span className="rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-semibold text-red-800 dark:border dark:border-red-800/60 dark:bg-red-950/60 dark:text-red-300">
              Devuelta total
            </span>
          );
        }
          ...
          <div className="flex items-center gap-1.5">
            ...
            {isReturnable && canWrite ? (
              <button
                ...
                disabled={disabled}
                className="inline-flex items-center gap-1 rounded-md border border-amber-300 bg-amber-50 px-2 py-1 text-xs font-medium text-amber-800 hover:bg-amber-100 disabled:opacity-50 dark:border-amber-800 dark:bg-amber-950/50 dark:text-amber-300 dark:hover:bg-amber-900/60"
  ...
  if (!rows.length) {
    return (
      <div className="rounded-lg border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-400">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th key={header.id} className="px-4 py-2.5 text-left font-semibold text-slate-700 dark:text-slate-300">
        ...
        <section className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--ui-overlay)] p-2 sm:p-4">
          <div className="flex max-h-[calc(100vh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-panel dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3 rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-xs sm:grid-cols-4 dark:border-slate-800 dark:bg-slate-800/50">
                <div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedPurchase.purchase.items?.map((item) => (
                    <div
                      key={item.id}
                      className="flex flex-col gap-1.5 rounded-xl border border-slate-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800 dark:bg-slate-800/50"
                    >
                      <div>
              ...
              {selectedPurchase.purchase.notes ? (
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-800/50 dark:text-slate-300">
              ...
              <div className="space-y-1 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-800 dark:bg-slate-800/50">
                <div className="flex justify-between text-xs text-slate-600 dark:text-slate-400">
                  <span>Subtotal neto:</span>
                  <span className="dark:text-slate-200">{currency.format(selectedPurchase.purchase.subtotal)}</span>
                </div>
                {selectedPurchase.purchase.vat_total ? (
                  <div className="flex justify-between text-xs text-slate-600 dark:text-slate-400">
                    <span>IVA total:</span>
                    <span className="dark:text-slate-200">{currency.format(selectedPurchase.purchase.vat_total)}</span>
                  </div>
                ) : null}
                <div className="flex justify-between text-base font-bold text-slate-900 dark:text-slate-100 border-t border-slate-200 dark:border-slate-700 pt-1">
